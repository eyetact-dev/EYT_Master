...
var nestableList = $("#admin_nestable > .dd-list");
...
var deleteFromMenuHelper = function (target) {
    if (target.data('new') == 1) {
        ...
        target.fadeOut(function () {
            target.remove();
            updateOutput($('#admin_nestable').data('output', $('#json-output')));
        });
    } else {
...
var addToMenu = function () {
    ...
    // set events
    $("#admin_nestable .button-delete").on("click", deleteFromMenu);
    $("#admin_nestable .button-edit").on("click", prepareEdit);
};
